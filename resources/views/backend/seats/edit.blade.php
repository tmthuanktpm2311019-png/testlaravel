 ...
 <head>
     ...
     <title>Chỉnh sửa Ghế - TVCinema</title>
 ...
 <body>
     <div id="wrapper">
         ...
         <ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar">
             ...
             <li class="nav-item"><a class="nav-link" href="{{ route('rooms.index') }}"><i
         ...
         <div id="content-wrapper" class="d-flex flex-column">
             <div id="content">
                 ...
                 <div class="container-fluid">
                     <h1 class="h3 mb-4 text-gray-800">Chỉnh sửa Ghế</h1>
                             ...
                             <form action="{{ route('seats.update', $seat->seat_id) }}" method="POST">
                                 ...
                                 <div class="form-group">
                                     ...
                                     <input type="text" class="form-control"
                                         value="{{ $seat->room->theater->name }}" readonly>
                                 </div>

                                 <div class="form-group">
                                     <label for="room_id">Phòng chiếu</label>
                                     <select class="form-control" id="room_id" name="room_id"
                                         {{ $seat->hasBookings ? 'disabled' : '' }}>
                                         @foreach ($rooms as $room)
                                             <option value="{{ $room->room_id }}"
                                                 {{ $seat->room_id == $room->room_id ? 'selected' : '' }}>
                                                 {{ $room->name }} - {{ $room->theater->name }}
                                             </option>
                                         @endforeach
                                     </select>
                                     @if ($seat->hasBookings)
                                         <small class="text-danger">⚠ Ghế này đã có vé đặt — không thể đổi
                                             phòng.</small>
                                     @endif
                                 </div>
                                 ...
                                 <div class="form-group">
                                     ...
                                     <select class="form-control" id="seat_type" name="seat_type" required>
                                         ...
                                         <option value="standard"
                                             {{ $seat->seat_type === 'standard' ? 'selected' : '' }}>Standard</option>
                                         <option value="vip" {{ $seat->seat_type === 'vip' ? 'selected' : '' }}>VIP
                                         </option>
                                     </select>
                                 </div>

                                 <button type="submit" class="btn btn-primary">Cập nhật Ghế</button>
                                 <a href="{{ route('seats.index') }}" class="btn btn-secondary">Hủy</a>
                             </form>
