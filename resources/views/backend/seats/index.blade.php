...
<head>
    ...
    <title>Quản lý Ghế - TVCinema</title>
...
<body>
    <div id="wrapper">
        ...
        <ul class="navbar-nav sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <li class="nav-item"><a class="nav-link" href="{{ route('rooms.index') }}"><i class="fa-solid fa-chair"></i>
                    <span>Phòng chiếu</span></a></li>
            <li class="nav-item"><a class="nav-link" href="{{ route('showtimes.index') }}"><i
                        class="fa-solid fa-clock"></i> <span>Suất chiếu</span></a></li>
            <li class="nav-item active"><a class="nav-link" href="{{ route('seats.index') }}"><i
            ...
            <li class="nav-item"><a class="nav-link" href="{{ route('users.index') }}"><i class="fa-solid fa-users"></i>
            ...
            <div id="content">
                        ...
                        <li class="nav-item dropdown no-arrow">
                            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button"
                                ...
                                <img class="avatar-img"
                                    src="{{ Auth::user()->avatar ?? asset('assets/img/avatar.jpg') }}" alt="avatar">
                            </a>
                ...
                <div class="container-fluid">
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Quản lý Ghế</h1>
                        <a href="{{ route('seats.create') }}" class="btn btn-primary shadow-sm">
                            <i class="fas fa-plus fa-sm text-white-50"></i> Thêm Ghế Mới
                        </a>
                    </div>

                    {{-- Bộ lọc Rạp / Phòng --}}
                    <form method="GET" action="{{ route('seats.index') }}" class="filter-form mb-3">
                        <label for="theater_id">Rạp:</label>
                        <select name="theater_id" id="theater_id" class="form-control d-inline-block"
                            onchange="updateRooms()">
                            <option value="">Tất cả</option>
                            @foreach ($theaters as $theater)
                                <option value="{{ $theater->theater_id }}"
                                    {{ request('theater_id') == $theater->theater_id ? 'selected' : '' }}>
                                    {{ $theater->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="room_id" id="room_id" class="form-control d-inline-block"
                            onchange="this.form.submit()">
                            <option value="">Tất cả</option>
                            @foreach ($rooms as $room)
                                @if (!request('theater_id') || $room->theater_id == request('theater_id'))
                                    ...
                                    <tbody>
                                        @forelse ($seats as $seat)
                                            <tr>
                                                ...
                                                <td>{{ $seat->room->theater->name }}</td>
                                                <td>
                                                    <a href="{{ route('seats.edit', $seat->seat_id) }}"
                                                        class="btn btn-warning btn-sm">Sửa</a>
                                                    <form action="{{ route('seats.destroy', $seat->seat_id) }}"
    ...
    <script>
        function updateRooms() {
            ...
            url.searchParams.set('theater_id', theaterId);
